...
class Client extends Controller
{
    ...
    private function instance()
    {
        try {
            ...
            if ($auth->isEmpty()) throw new Exception("该公众号还未授权，请重新授权！");
            if (empty($auth['status'])) throw new Exception('该公众号已被禁用，请联系管理员！');
            if (!empty($auth['deleted'])) throw new Exception('该公众号已取消授权，请重新授权！');
            if (abs(time() - $data['time']) > 3600) throw new Exception('请求时间与服务器时差过大，请同步时间！');
            if (md5("{$class}#{$appid}#{$auth['appkey']}#{$time}#{$nostr}") !== $sign) throw new Exception("该公众号{$appid}请求签名异常！");
